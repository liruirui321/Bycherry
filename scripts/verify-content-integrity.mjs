...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('minHeight: "calc(100svh - 50px)"') && heroSource.includes('alignItems: "start"') && heroSource.includes('alignContent: "start"') && heroSource.includes('padding: "clamp(1.2rem, 2.8vh, 2rem)') && heroSource.includes('margin: "0 auto"') && heroSource.includes('gridTemplateColumns: "minmax(520px, 0.36fr) minmax(920px, 0.64fr)"') && heroSource.includes("maxWidth: 1880") && !heroSource.includes('height: "calc(100vh - 116px)"') && !heroSource.includes('margin: "clamp(-4.2rem') && !heroSource.includes('transform: "translateY('), "Homepage hero must return to the original studio feel while starting near the top of the first viewport.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(3, minmax(0, 1fr))"') && heroSource.includes('gap: "0.78rem"'), "Homepage module entries must render as a compact first-screen grid, not repeated sections.");
  expect(heroSource.includes("borderLeft: `0.5rem solid ${entry.border}`") && heroSource.includes("minHeight: 188") && heroSource.includes("linear-gradient(135deg, ${entry.color}") && heroSource.includes(".hero-entry-card::after") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep science-card density without returning to overly long sticky-note panels.");
