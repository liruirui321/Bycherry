...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes("hero-entry-desc") && heroSource.includes("-webkit-line-clamp: 2") && heroSource.includes("-webkit-line-clamp: 1 !important"), "Hero directory cards must show compact clamped module descriptions without becoming long pages.");
  expect(heroSource.includes('minHeight: "calc(100vh - 50px)"') && heroSource.includes('alignItems: "center"') && heroSource.includes('alignContent: "center"') && heroSource.includes('gridTemplateColumns: "minmax(290px, 0.52fr) minmax(760px, 1.48fr)"') && heroSource.includes("maxWidth: 1840") && !heroSource.includes('height: "calc(100vh - 116px)"'), "Homepage hero must keep the original landing feel with a balanced first-screen content directory instead of empty slogan space.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(2, minmax(0, 1fr))"') && heroSource.includes('gap: "0.72rem"'), "Homepage module entries must render as a compact first-screen grid, not repeated sections.");
  expect(heroSource.includes("borderLeft: `0.45rem solid ${entry.border}`") && heroSource.includes("minHeight: 156") && !heroSource.includes("minHeight: 224") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep enough content density without returning to overly long sticky-note panels.");
