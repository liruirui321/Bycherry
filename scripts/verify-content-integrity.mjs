...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(320px, 0.72fr) minmax(0, 1.28fr)"') && heroSource.includes("hero-soft-blob") && heroSource.includes("hero-stem"), "Hero must keep the original two-column studio composition with visible background atmosphere.");
  ...
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(3, minmax(0, 1fr))"') && heroSource.includes('gridAutoRows: "minmax(178px, auto)"'), "Homepage module entries must render as a dense three-column first-screen directory, not repeated sections.");
  expect(heroSource.includes("border: `2px solid ${entry.border}`") && heroSource.includes("minHeight: 178") && !heroSource.includes("minHeight: 230") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep enough content density without returning to long sticky-note panels.");
