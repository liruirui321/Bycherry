...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(300px, 0.82fr) minmax(520px, 1.18fr)"') && heroSource.includes("hero-featured-panel") && heroSource.includes("内容目录") && heroSource.includes("hero-entry-list"), "Hero must use one original-studio first-screen layout with a directly visible compact content directory.");
  ...
  expect(heroSource.includes('minHeight: "calc(100vh - 50px)"') && heroSource.includes('gridTemplateColumns: "minmax(300px, 0.82fr) minmax(520px, 1.18fr)"') && heroSource.includes("maxWidth: 1280"), "Homepage hero must keep the original landing feel while using the first viewport for real content instead of empty slogan space.");
  expect(heroSource.includes('className="hero-entry-list"') && heroSource.includes('display: "grid"') && heroSource.includes('gap: "0.48rem"'), "Homepage module entries must render as a compact first-screen list, not repeated sections.");
