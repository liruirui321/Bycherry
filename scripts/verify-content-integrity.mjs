...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(300px, 0.5fr) minmax(780px, 1.5fr)"') && heroSource.includes("hero-featured-panel") && heroSource.includes("精选内容") && heroSource.includes("hero-entry-grid"), "Hero must use one original-studio first-screen layout with a directly visible compact content directory.");
  ...
  expect(heroSource.includes("grid-template-columns: 2.1rem minmax(0, calc(100vw - 7.6rem)) !important") && heroSource.includes("max-width: calc(100vw - 4rem) !important") && heroSource.includes("justify-self: start !important") && heroSource.includes("gap: 0.5rem !important") && heroSource.includes("overflow: hidden !important") && heroSource.includes(".hero-entry-desc") && heroSource.includes("-webkit-line-clamp: 2 !important") && heroSource.includes("overflow-wrap: anywhere !important"), "Homepage mobile rows must keep compact module titles and wrapped context without right-edge overflow.");
  ...
  expect(heroSource.includes('minHeight: "calc(100svh - 50px)"') && heroSource.includes('alignItems: "center"') && heroSource.includes('alignContent: "center"') && heroSource.includes('padding: "clamp(2.1rem, 4.2vh, 3rem)') && heroSource.includes('margin: "0 auto"') && heroSource.includes('gridTemplateColumns: "minmax(300px, 0.5fr) minmax(780px, 1.5fr)"') && heroSource.includes("maxWidth: 1840") && !heroSource.includes('height: "calc(100vh - 116px)"') && !heroSource.includes('margin: "clamp(-4.2rem') && !heroSource.includes('transform: "translateY('), "Homepage hero must keep the original studio feel while filling the first viewport with a complete content directory.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(6, minmax(0, 1fr))"') && heroSource.includes('gap: "0.82rem"'), "Homepage module entries must render as a compact first-screen grid, not repeated sections.");
  expect(heroSource.includes("borderLeft: `0.45rem solid ${entry.border}`") && heroSource.includes("minHeight: 198") && !heroSource.includes("minHeight: 224") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep enough content density without returning to overly long sticky-note panels.");
...
function verifyWorkJsonLdLearningOutcomes() {
  ...
  expect(heroSource.includes(".hero-entry-grid") && heroSource.includes("justify-items: start !important") && heroSource.includes("grid-template-columns: 2.1rem minmax(0, calc(100vw - 7.6rem)) !important") && heroSource.includes("padding: 0.58rem 0.64rem !important") && heroSource.includes("-webkit-line-clamp: 2 !important") && heroSource.includes(".hero-entry-side") && heroSource.includes("display: none !important"), "Homepage hero mobile featured directory must remain short while fitting the visible viewport.");
