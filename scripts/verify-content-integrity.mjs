...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes("A tiny studio for") && heroSource.includes("hero-soft-blob") && heroSource.includes("hero-stem") && heroSource.includes("hero-float-deco") && heroSource.includes("hero-scope-strip"), "Hero must recover the original studio atmosphere while keeping useful first-screen density.");
  ...
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(3, minmax(0, 1fr))"') && heroSource.includes('gridAutoRows: "minmax(104px, auto)"'), "Homepage module entries must render as a short three-column first-screen index, not repeated sections.");
  expect(heroSource.includes("border: `2px solid ${entry.border}`") && heroSource.includes("minHeight: 104") && !heroSource.includes("minHeight: 152") && !heroSource.includes("minHeight: 230") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep enough content density without returning to long sticky-note panels.");
  expect(!heroSource.includes('gridRow: entry.featuredImage ? "span 2" : undefined') && heroSource.includes("width: 70") && heroSource.includes("height: 52"), "Homepage plant evolution entry must show a compact illustration without becoming a two-row sticky-note card.");
