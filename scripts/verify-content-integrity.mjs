...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes("A tiny studio for") && heroSource.includes("hero-forest-blob") && heroSource.includes("hero-fern") && heroSource.includes("hero-float-deco"), "Hero must return to the original studio cover atmosphere.");
  expect(heroSource.includes("hero-main") && heroSource.includes("hero-badge") && heroSource.includes("hero-title-science") && heroSource.includes("浏览主题作品") && heroSource.includes("读读笔记"), "Hero must keep an original-style cover with two primary CTAs.");
  expect(heroSource.includes("hero-layout") && heroSource.includes("grid-template-columns: minmax(390px, 0.9fr) minmax(620px, 1.35fr)") && heroSource.includes("hero-showcase") && heroSource.includes("精选内容"), "Homepage content directory must sit beside the cover copy in the first viewport.");
  expect(heroSource.includes("min-height: calc(100svh - 56px)") && heroSource.includes("width: min(100%, 1520px)") && heroSource.includes("grid-template-columns: repeat(2, minmax(0, 1fr))") && !heroSource.includes("hero-featured-panel") && !heroSource.includes("hero-entry-card-wide"), "Homepage entries must use a balanced two-column showcase instead of an empty lower index.");
  expect(heroSource.includes("hero-entry-copy strong") && heroSource.includes("-webkit-line-clamp: 2") && heroSource.includes("aria-label={`打开${entry.title}：${entry.desc}`}") && heroSource.includes("{entry.desc}</span>") && !heroSource.includes("entry.task"), "Homepage entries may show short real descriptions while preserving full descriptions in assistive labels.");
  expect(!heroSource.includes("hero-entry-reading") && heroSource.includes('const readingHref = "/reading"'), "Homepage reading should stay as one CTA instead of a repeated reading-card entry.");
  expect(heroSource.includes("hero-layout") && !heroSource.includes("hero-quick-index") && !heroSource.includes("hero-flow-board") && !heroSource.includes("hero-workbench-visual"), "Hero must keep the restored cover layout without repeated side indexes or workbench filler panels.");
  expect(!heroSource.includes("hero-entry-desc") && !heroSource.includes("heroMobileDescriptions") && !heroSource.includes("hero-entry-desc-full") && !heroSource.includes("hero-entry-desc-short"), "Homepage module cards should not carry separate desktop/mobile long descriptions.");
  expect(!heroSource.includes('gridRow: entry.featuredImage ? "span 2" : undefined') && !heroSource.includes("featuredImage") && !heroSource.includes("hero-entry-side"), "Homepage plant evolution entry must stay a compact directory item instead of a two-row illustration card.");
  ...
  expect(heroSource.includes("entry.outputs.slice(0, 2)") && !heroSource.includes("hero-entry-path") && !heroSource.includes("entry.path.map") && !heroSource.includes("hero-entry-output-list") && !heroSource.includes("hero-entry-task") && !heroSource.includes("task: work.task"), "Homepage cards must show only compact output cues without restoring long task/path blocks.");
...
function verifyWorkJsonLdLearningOutcomes() {
  ...
  expect(heroSource.includes("hero-entry-card") && heroSource.includes("display: grid") && !heroSource.includes("hero-featured-panel") && !heroSource.includes("hero-work-card"), "Homepage hero module entries must use one compact directory, not repeated card sections.");
  expect(!heroSource.includes("hero-entry-desc") && !heroSource.includes("hero-entry-description-card") && heroSource.includes("hero-entry-copy"), "Homepage hero can expose compact output cues but not long description blocks.");
  ...
  expect(heroSource.includes("@media (max-width: 480px)") && heroSource.includes("grid-template-columns: 1fr") && heroSource.includes("min-height: 66px") && heroSource.includes(".hero-entry-copy span,") && heroSource.includes(".hero-entry-meta") && heroSource.includes("display: none;"), "Homepage hero mobile directory must remain short while fitting the visible viewport.");
  expect(heroSource.includes("box-sizing: border-box"), "Homepage hero containers must use border-box sizing to avoid mobile overflow.");
...
function verifyResearchAgentWorkbenchContract() {
  ...
  const requiredWorkbenchFeatures = [
    ...
    { label: "first-screen output map data", text: "researchOutputMap" },
    { label: "first-screen output map", text: "research-output-map" },
    { label: "output map card", text: "research-output-map-card" },
    { label: "copyable product output", text: "研究记录 + 引用核查" },
...
function verifyConceptExplainerAgentContract() {
  ...
  const requiredConceptFeatures = [
    ...
    { label: "first-screen visual strip", text: "concept-live-visual-strip" },
    { label: "first-screen visual nodes", text: "concept-live-visual-node" },
  ...
  expect(conceptSource.includes("#concept-explainer-tool .concept-pack-card-grid") && conceptSource.includes("grid-template-columns: repeat(2, minmax(0, 1fr)) !important") && conceptSource.includes("@media (max-width: 420px)") && conceptSource.includes("grid-template-columns: 1fr !important") && conceptSource.includes("#concept-explainer-tool *") && conceptSource.includes("overflow-wrap: anywhere !important") && conceptSource.includes("concept-pack-copy-button") && conceptSource.includes("width: 100% !important"), "Concept explainer mobile explanation pack must stay readable as two-column cards, collapse to one column on narrow phones, and keep a full-width copy action.");
  ...
  expect(conceptSource.includes('className="concept-explanation-pack concept-live-result-panel"') && conceptSource.includes('background: "rgba(250,247,241,0.72)"') && conceptSource.includes('gridTemplateColumns: "repeat(auto-fit, minmax(132px, 1fr))"') && conceptSource.includes("minHeight: 88") && conceptSource.includes("当前结果") && conceptSource.includes('className="concept-live-visual-strip"') && conceptSource.includes('aria-label={`${concept} 的首屏可视化解释`}'), "Concept explainer desktop explanation pack must show short result tiles plus a first-screen visual flow inside the input panel.");
  expect(conceptSource.indexOf('className="concept-explanation-pack concept-live-result-panel"') > conceptSource.indexOf('className="concept-quick-start-strip"') && conceptSource.indexOf('className="concept-explanation-pack concept-live-result-panel"') < conceptSource.indexOf('className="concept-freeform-brief-panel"'), "Concept explainer live result must appear before the long input form so the first screen shows product output, not just fields.");
  expect(conceptSource.indexOf('className="concept-live-visual-strip"') > conceptSource.indexOf('className="concept-pack-card-grid"') && conceptSource.indexOf('className="concept-live-visual-strip"') < conceptSource.indexOf('className="concept-freeform-brief-panel"'), "Concept explainer first-screen visual flow must appear before the long input form.");
...
function verifyGeneExpressionLearnerContract() {
  ...
  const requiredGeneFeatures = [
    ...
    { label: "coupled expression lens", text: "CoupledExpressionLens" },
    { label: "visible process lens", text: "gene-coupled-expression-lens" },
    { label: "lens peptide bead chain", text: "gene-coupled-lens-peptide" },
    { label: "lens transcription-translation wording", text: "3' 端跟聚合酶，5' 端先被核糖体读取" },
  ...
  expect(geneSource.includes('className="gene-process-legend"') && geneSource.includes('transform="translate(742 34)"') && !geneSource.includes('transform="translate(660 40)"'), "Gene expression canvas legend must stay clear of the top process rail.");
...
function verifyLearnerFacingArticleCopy() {
  ...
  expect(articleLibrarySource.includes('background: "rgba(250,247,241,0.58)"') && articleLibrarySource.includes('border: "1px solid rgba(94,68,42,0.1)"') && articleLibrarySource.includes('borderLeft: "3px solid rgba(93,140,101,0.58)"') && articleLibrarySource.includes('borderRadius: 8') && !articleLibrarySource.includes('boxShadow: "0 8px 18px rgba(94,68,42,0.04)"') && !articleLibrarySource.includes("minHeight: 166"), "Reading library entries must stay as compact clickable rows instead of thick cards.");
  ...
  expect(articleLibrarySource.includes('background: "rgba(250,247,241,0.58)"') && articleLibrarySource.includes('borderLeft: "3px solid rgba(93,140,101,0.58)"') && articleLibrarySource.includes("article-library-row-arrow") && articleLibrarySource.includes("transform: translateY(-1px)"), "Reading library article rows must look like compact clickable task entries rather than a passive text list.");
...
function verifyLearnerProductPositioning() {
  ...
  for (const requiredNavEntry of ['label: "主题作品"', 'label: "科学模拟"', 'label: "科研"', 'label: "小工具"', 'label: "笔记"', 'href: "/works/gene-expression"', 'href: "/works/research-prompt-kit"', 'href: "/works/concept-explainer"', 'href: "/reading"']) {
