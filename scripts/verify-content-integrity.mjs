...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes("hero-main") && heroSource.includes("hero-badge") && heroSource.includes("hero-title-science") && heroSource.includes("看看主题作品") && heroSource.includes("读读笔记"), "Hero must keep a compact original-style cover with two primary CTAs.");
  expect(heroSource.includes("grid-template-rows: minmax(0, 1fr) auto") && heroSource.includes("grid-template-columns: repeat(6, minmax(0, 1fr))") && heroSource.includes("@media (max-width: 1100px)") && heroSource.includes("repeat(3, minmax(0, 1fr))") && heroSource.includes("@media (max-width: 480px)") && heroSource.includes("grid-template-columns: 1fr"), "Homepage content directory must sit as a compact bottom index in the first viewport.");
  expect(heroSource.includes("min-height: calc(100svh - 56px)") && heroSource.includes("width: min(100%, 1220px)") && heroSource.includes("min-height: 86px") && !heroSource.includes("hero-featured-panel") && !heroSource.includes("精选内容") && !heroSource.includes("hero-entry-card-wide"), "Homepage entries must avoid the oversized right-side card grid.");
  expect(heroSource.includes("hero-entry-copy strong") && heroSource.includes("white-space: nowrap") && heroSource.includes("text-overflow: ellipsis") && heroSource.includes("aria-label={`打开${entry.title}：${entry.desc}`}") && !heroSource.includes("{entry.desc}</span>") && !heroSource.includes("entry.task"), "Homepage entries must stay short visually while keeping full descriptions in assistive labels.");
  expect(heroSource.includes("hero-entry-reading") && heroSource.includes("科研证据 / 学习方法") && heroSource.includes('const readingHref = "/reading"'), "Homepage may include one compact reading-card entry without repeating article lists.");
  expect(!heroSource.includes("hero-layout") && !heroSource.includes("hero-quick-index") && !heroSource.includes("hero-flow-board") && !heroSource.includes("hero-workbench-visual"), "Hero must not reintroduce repeated side indexes or workbench filler panels.");
...
function verifyWorkJsonLdLearningOutcomes() {
  ...
  expect(heroSource.includes("@media (max-width: 480px)") && heroSource.includes("grid-template-columns: 1fr") && heroSource.includes("min-height: 66px") && heroSource.includes("text-overflow: ellipsis"), "Homepage hero mobile directory must remain short while fitting the visible viewport.");
