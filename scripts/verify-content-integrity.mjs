...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(320px, 0.48fr) minmax(0, 1fr)"') && heroSource.includes("hero-featured-panel") && heroSource.includes("精选内容") && heroSource.includes("内容目录"), "Hero must use one original-studio first-screen layout with a directly visible content directory.");
  ...
  expect(heroSource.includes('className="hero-title-break"') && heroSource.includes('className="hero-title-mobile-break"') && heroSource.includes("font-size: 1.58rem !important"), "Homepage hero title must have explicit mobile-safe breaks and small-screen font size.");
  expect(heroSource.includes(".hero-inner,") && heroSource.includes(".hero-featured-panel,") && !heroSource.includes(".hero-summary-panel,") && heroSource.includes("width: calc(100vw - 3rem) !important"), "Homepage mobile layout must keep content inside the viewport instead of clipping at the right edge.");
  expect(heroSource.includes("justify-items: stretch !important") && heroSource.includes("max-width: 20rem !important") && heroSource.includes("justify-self: start !important") && heroSource.includes("-webkit-line-clamp: 2 !important"), "Homepage mobile cards must keep readable wrapped descriptions without single-line clipping or right-edge overflow.");
  ...
  expect(heroSource.includes('minHeight: "calc(100vh - 74px)"') && heroSource.includes('height: "calc(100vh - 116px)"') && heroSource.includes('gridTemplateColumns: "minmax(320px, 0.48fr) minmax(0, 1fr)"') && heroSource.includes("maxWidth: 1580"), "Homepage hero must keep the original landing feel while using the lower viewport for real content instead of empty slogan space.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(3, minmax(0, 1fr))"') && heroSource.includes('gridAutoRows: "minmax(158px, 1fr)"'), "Homepage module entries must render as a compact three-column first-screen matrix, not repeated sections.");
  expect(heroSource.includes("border: `2px solid ${entry.border}`") && heroSource.includes("minHeight: 158") && !heroSource.includes("minHeight: 224") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep enough content density without returning to overly long sticky-note panels.");
  expect(!heroSource.includes('gridRow: entry.featuredImage ? "span 2" : undefined') && heroSource.includes("width: 78") && heroSource.includes("height: 66"), "Homepage plant evolution entry must show a compact illustration without becoming a two-row sticky-note card.");
  ...
  expect(heroSource.includes("hero-entry-path") && heroSource.includes("entry.path.map") && heroSource.includes("entry.outputs.slice(0, 3)") && heroSource.includes("hero-entry-task") && heroSource.includes("task: work.task"), "Homepage cards must preserve useful learning-path, task and output density after removing repeated action pills.");
...
function verifyWorkJsonLdLearningOutcomes() {
  ...
  expect(heroSource.includes(".hero-entry-grid") && heroSource.includes("grid-template-columns: repeat(2, minmax(0, 1fr)) !important") && heroSource.includes("grid-template-columns: 1fr !important") && heroSource.includes("padding: 0.56rem !important") && heroSource.includes(".hero-entry-row-image img") && heroSource.includes("display: none !important"), "Homepage hero mobile featured directory must remain short while fitting the visible viewport.");
