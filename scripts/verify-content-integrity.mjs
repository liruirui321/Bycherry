...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(290px, 0.52fr) minmax(760px, 1.48fr)"') && heroSource.includes("hero-featured-panel") && heroSource.includes("精选内容") && heroSource.includes("hero-entry-grid"), "Hero must use one original-studio first-screen layout with a directly visible compact content directory.");
  expect(heroSource.includes('gridTemplateColumns: "repeat(6, minmax(0, 1fr))"') && heroSource.includes('gridColumn: index >= 3 ? "span 3" : "span 2"') && heroSource.includes("个内容"), "Homepage work directory must fill the first screen as 3 compact cards over 2 wider cards instead of leaving an empty right-bottom area.");
  ...
  expect(heroSource.includes("grid-template-columns: 2.1rem minmax(0, 1fr) !important") && heroSource.includes("max-width: 100% !important") && heroSource.includes("gap: 0.5rem !important") && heroSource.includes("overflow: hidden !important") && heroSource.includes(".hero-entry-desc") && heroSource.includes("display: none !important") && heroSource.includes("text-overflow: ellipsis !important"), "Homepage mobile rows must keep compact module titles without right-edge overflow.");
  ...
  expect(heroSource.includes("hero-entry-desc") && heroSource.includes("-webkit-line-clamp: 2") && heroSource.includes(".hero-entry-desc") && heroSource.includes("display: none !important"), "Hero directory cards must show compact descriptions on desktop and hide them on narrow mobile rows.");
  expect(heroSource.includes('minHeight: "auto"') && heroSource.includes('alignItems: "start"') && heroSource.includes('alignContent: "start"') && heroSource.includes('padding: "clamp(2.4rem, 5vh, 3.2rem)') && heroSource.includes('margin: "0 auto"') && heroSource.includes('gridTemplateColumns: "minmax(290px, 0.52fr) minmax(760px, 1.48fr)"') && heroSource.includes("maxWidth: 1840") && !heroSource.includes('height: "calc(100vh - 116px)"') && !heroSource.includes('minHeight: "calc(100vh - 50px)"') && !heroSource.includes('margin: "clamp(-4.2rem') && !heroSource.includes('transform: "translateY('), "Homepage hero must keep the original studio feel without forcing an empty full-screen first viewport.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(6, minmax(0, 1fr))"') && heroSource.includes('gap: "0.72rem"'), "Homepage module entries must render as a compact first-screen grid, not repeated sections.");
  expect(heroSource.includes("borderLeft: `0.45rem solid ${entry.border}`") && heroSource.includes("minHeight: 162") && !heroSource.includes("minHeight: 180") && !heroSource.includes("minHeight: 224") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep enough content density without returning to overly long sticky-note panels.");
...
function verifyWorkJsonLdLearningOutcomes() {
  ...
  expect(heroSource.includes(".hero-entry-grid") && heroSource.includes("grid-template-columns: 2.1rem minmax(0, 1fr) !important") && heroSource.includes("padding: 0.52rem 0.58rem !important") && heroSource.includes(".hero-entry-side") && heroSource.includes("display: none !important"), "Homepage hero mobile featured directory must remain short while fitting the visible viewport.");
