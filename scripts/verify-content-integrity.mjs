...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes("A tiny studio for") && heroSource.includes("hero-soft-blob") && heroSource.includes("hero-stem") && heroSource.includes("hero-float-deco") && heroSource.includes("hero-start-strip"), "Hero must recover the original studio atmosphere while keeping useful first-screen density.");
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(300px, 0.58fr) minmax(720px, 1.42fr)"') && heroSource.includes("maxWidth: 1480"), "Homepage must use a wider first-screen directory layout instead of leaving the page mostly empty.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(3, minmax(0, 1fr))"') && heroSource.includes('gridAutoRows: "minmax(124px, auto)"'), "Homepage module entries must render as a short three-column first-screen index, not repeated sections.");
  expect(heroSource.includes("border: `2px solid ${entry.border}`") && heroSource.includes("minHeight: 124") && !heroSource.includes("minHeight: 152") && !heroSource.includes("minHeight: 230") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep enough content density without returning to long sticky-note panels.");
  ...
  expect(heroSource.includes('aria-label="使用方式"') && heroSource.includes("先选一个卡住的问题") && !heroSource.includes("hero-scope-strip") && !heroSource.includes("hero-scope-item"), "Homepage should use one compact usage strip instead of a repeated category scope row.");
  ...
  expect(!heroSource.includes("hero-count") && !heroSource.includes("homeWorkStatus") && !heroSource.includes("hero-work-status") && !heroSource.includes("个内容入口"), "Homepage first-screen directory must not add duplicate count or status labels.");
  ...
  expect(heroSource.includes("hero-entry-path") && heroSource.includes("entry.path.map") && heroSource.includes("entry.outputs.slice(0, 3)"), "Homepage cards must preserve useful learning-path and output density after removing repeated action pills.");
...
function verifyLearnerFacingArticleCopy() {
  ...
  expect(articleLibrarySource.includes("article-library-start-strip") && articleLibrarySource.includes("readingWorkflow") && articleLibrarySource.includes("选卡点") && articleLibrarySource.includes("带着问题读") && articleLibrarySource.includes("留下记录"), "Reading library must show a compact start path so articles are easy to choose and use.");
  expect(articleLibrarySource.includes("articleChoiceRoutes") && articleLibrarySource.includes("article-choice-route-strip") && articleLibrarySource.includes("按卡点选文章"), "Reading library must expose a compact article choice route strip.");
  ...
  expect(articleLibrarySource.includes(".article-choice-route-strip > div") && articleLibrarySource.includes("grid-template-columns: repeat(2, minmax(0, 1fr)) !important"), "Reading library choice route must stay compact on mobile.");
