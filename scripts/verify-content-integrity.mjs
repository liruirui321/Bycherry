...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(420px, 0.48fr) minmax(660px, 0.72fr)"') && heroSource.includes("hero-featured-panel") && heroSource.includes("精选内容") && heroSource.includes("hero-entry-grid"), "Hero must keep the original-studio first-screen layout while exposing a compact directly visible content directory.");
  expect(heroSource.includes('gridTemplateColumns: "repeat(4, minmax(0, 1fr))"') && heroSource.includes('gridColumn: "span 2"') && heroSource.includes('gridRow: index === 0 ? "span 2" : undefined') && heroSource.includes("个内容"), "Homepage work directory must use a balanced two-column index with one featured simulation plus four compact modules.");
  ...
  expect(heroSource.includes('minHeight: "calc(100svh - 50px)"') && heroSource.includes('alignItems: "center"') && heroSource.includes('alignContent: "center"') && heroSource.includes('padding: "clamp(2.4rem, 5vh, 3.4rem)') && heroSource.includes('margin: "0 auto"') && heroSource.includes('gridTemplateColumns: "minmax(420px, 0.48fr) minmax(660px, 0.72fr)"') && heroSource.includes("maxWidth: 1480") && !heroSource.includes('height: "calc(100vh - 116px)"') && !heroSource.includes('margin: "clamp(-4.2rem') && !heroSource.includes('transform: "translateY('), "Homepage hero must keep the original studio feel while fitting a complete content directory into the first viewport.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(4, minmax(0, 1fr))"') && heroSource.includes('gap: "0.72rem"'), "Homepage module entries must render as a compact first-screen grid, not repeated sections.");
  expect(heroSource.includes("borderLeft: `0.5rem solid ${entry.border}`") && heroSource.includes("minHeight: 154") && heroSource.includes("linear-gradient(135deg, ${entry.color}") && heroSource.includes(".hero-entry-card::after") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep science-card density without returning to overly long sticky-note panels.");
  expect(heroSource.includes("width: 10.2rem") && heroSource.includes("height: 3.6rem") && heroSource.includes(".hero-entry-card:first-child {") && heroSource.includes("grid-template-rows: auto auto auto !important") && heroSource.includes("hero-entry-focus-line") && heroSource.includes("固定序列 ATG GAA TTT CCG") && heroSource.includes(".hero-entry-card:first-child .hero-mini-visual") && heroSource.includes("height: 6.35rem") && heroSource.includes(".hero-entry-card:first-child .hero-mini-visual-gene .hero-mini-chain i") && heroSource.includes("width: 0.62rem"), "Homepage mini science visuals must be compact illustrated cues instead of oversized blank areas.");
