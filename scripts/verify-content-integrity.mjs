...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes("const firstArticle = articleLinks[0]") && heroSource.includes('href={firstArticle?.href ?? "/"}') && !heroSource.includes("hero-article-row") && !heroSource.includes("articleLinks.map((article)"), "Homepage must not expand the reading library into a long list of article entries.");
...
function verifyWorkJsonLdLearningOutcomes() {
  ...
  expect(heroSource.includes("hero-reading-library") && heroSource.includes("firstArticle") && !heroSource.includes("hero-article-row"), "Homepage hero must expose the reading library as one compact entry, not an expandable article list.");
