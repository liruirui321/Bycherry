...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(270px, 0.38fr) minmax(900px, 1.62fr)"') && heroSource.includes("hero-featured-panel") && heroSource.includes("精选内容") && heroSource.includes("hero-entry-grid"), "Hero must use one original-studio first-screen layout with a wider directly visible content directory and a tighter title column.");
  expect(heroSource.includes('gridTemplateColumns: "repeat(6, minmax(0, 1fr))"') && heroSource.includes('gridColumn: "span 2"') && heroSource.includes('gridRow: index === 0 ? "span 2" : undefined') && heroSource.includes("个内容"), "Homepage work directory must fill the first screen as one featured simulation plus four compact modules instead of leaving an empty right-bottom area.");
  ...
  expect(heroSource.includes('minHeight: "calc(100svh - 50px)"') && heroSource.includes('alignItems: "center"') && heroSource.includes('alignContent: "center"') && heroSource.includes('padding: "clamp(2.1rem, 4.2vh, 3rem)') && heroSource.includes('margin: "0 auto"') && heroSource.includes('gridTemplateColumns: "minmax(270px, 0.38fr) minmax(900px, 1.62fr)"') && heroSource.includes("maxWidth: 1900") && !heroSource.includes('height: "calc(100vh - 116px)"') && !heroSource.includes('margin: "clamp(-4.2rem') && !heroSource.includes('transform: "translateY('), "Homepage hero must keep the original studio feel while filling the first viewport with a complete content directory.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(6, minmax(0, 1fr))"') && heroSource.includes('gap: "0.78rem"'), "Homepage module entries must render as a compact first-screen grid, not repeated sections.");
  expect(heroSource.includes("borderLeft: `0.5rem solid ${entry.border}`") && heroSource.includes("minHeight: 218") && heroSource.includes("linear-gradient(135deg, ${entry.color}") && heroSource.includes(".hero-entry-card::after") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep stronger science-card density without returning to overly long sticky-note panels.");
  expect(heroSource.includes("width: 10.2rem") && heroSource.includes("height: 5.05rem") && heroSource.includes(".hero-entry-card:first-child {") && heroSource.includes("grid-template-rows: auto auto auto !important") && heroSource.includes("hero-entry-focus-line") && heroSource.includes("固定序列 ATG GAA TTT CCG") && heroSource.includes(".hero-entry-card:first-child .hero-mini-visual") && heroSource.includes("height: 8.9rem") && heroSource.includes(".hero-entry-card:first-child .hero-mini-visual-gene .hero-mini-chain i") && heroSource.includes("width: 0.62rem"), "Homepage mini science visuals must be large enough to make the cards feel illustrated instead of blank.");
