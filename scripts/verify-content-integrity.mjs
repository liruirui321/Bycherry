...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(370px, 0.35fr) minmax(0, 0.65fr)"') && heroSource.includes("hero-featured-panel") && heroSource.includes("精选内容") && heroSource.includes("hero-entry-grid") && heroSource.includes("maxWidth: 1660"), "Hero must keep the original-studio first-screen layout while exposing a compact directly visible content directory.");
  expect(heroSource.includes('gridTemplateColumns: "repeat(2, minmax(0, 1fr))"') && heroSource.includes("@media (min-width: 1500px)") && heroSource.includes("repeat(3, minmax(0, 1fr))") && heroSource.includes("hero-entry-card-wide") && !heroSource.includes('gridColumn: index === 0 ? "span 2" : undefined') && !heroSource.includes('gridRow: index === 0 ? "span 2" : undefined') && heroSource.includes("个内容"), "Homepage work directory must use a dense responsive index instead of leaving the first screen visually empty.");
  ...
  expect(heroSource.includes('minHeight: "calc(100svh - 56px)"') && heroSource.includes('alignItems: "center"') && heroSource.includes('alignContent: "center"') && heroSource.includes('padding: "clamp(1.05rem, 2.4vh, 1.8rem)') && heroSource.includes('margin: "0 auto"') && heroSource.includes('gridTemplateColumns: "minmax(370px, 0.35fr) minmax(0, 0.65fr)"') && heroSource.includes("maxWidth: 1660") && heroSource.includes('paddingTop: "clamp(4.7rem, 8.5vh, 5.7rem)"') && !heroSource.includes('height: "calc(100vh - 116px)"') && !heroSource.includes('margin: "clamp(-4.2rem') && !heroSource.includes('transform: "translateY('), "Homepage hero must return to the original studio feel while starting near the top of the first viewport.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(2, minmax(0, 1fr))"') && heroSource.includes('gap: "0.78rem"') && heroSource.includes("@media (min-width: 1500px)") && heroSource.includes("repeat(3, minmax(0, 1fr))"), "Homepage module entries must render as a compact first-screen grid, not repeated sections.");
  expect(heroSource.includes("borderLeft: `0.5rem solid ${entry.border}`") && heroSource.includes("minHeight: 0") && heroSource.includes("linear-gradient(135deg, ${entry.color}") && heroSource.includes(".hero-entry-card::after") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep science-card density without returning to overly long sticky-note panels.");
  ...
  expect(heroSource.includes("width: 10.2rem") && heroSource.includes("height: 3.6rem") && heroSource.includes("hero-entry-card-wide") && heroSource.includes("grid-template-columns: 2.4rem minmax(0, 1fr) minmax(250px, 0.78fr) !important") && heroSource.includes("hero-entry-focus-line") && heroSource.includes("固定序列 ATG GAA TTT CCG") && heroSource.includes(".hero-mini-visual-gene .hero-mini-chain i") && heroSource.includes("width: 0.48rem"), "Homepage mini science visuals must be compact illustrated cues instead of oversized blank areas.");
