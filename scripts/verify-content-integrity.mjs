...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes(".hero-inner,") && heroSource.includes(".hero-featured-panel,") && heroSource.includes(".hero-entry-list") && !heroSource.includes(".hero-summary-panel,") && heroSource.includes("width: calc(100vw - 1.7rem) !important"), "Homepage mobile layout must keep content inside the viewport instead of clipping at the right edge.");
  expect(heroSource.includes("grid-template-columns: 2.1rem minmax(0, 1fr) !important") && heroSource.includes("width: 100% !important") && heroSource.includes("-webkit-line-clamp: 1 !important"), "Homepage mobile rows must keep readable wrapped descriptions without right-edge overflow.");
