...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes(".hero-inner,") && heroSource.includes(".hero-heading-strip,") && heroSource.includes(".hero-featured-panel,") && heroSource.includes(".hero-summary-panel,") && heroSource.includes("width: calc(100vw - 3rem) !important"), "Homepage mobile layout must keep content inside the viewport instead of clipping at the right edge.");
  ...
  expect(heroSource.includes('minHeight: "auto"') && heroSource.includes('gridTemplateColumns: "minmax(0, 1fr) minmax(240px, 0.34fr)"') && heroSource.includes("maxWidth: 1720"), "Homepage header must be a compact top strip, not a large empty slogan area.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(3, minmax(0, 1fr))"') && heroSource.includes('gridAutoRows: "minmax(184px, 1fr)"'), "Homepage module entries must render as a compact three-column first-screen matrix, not repeated sections.");
  expect(heroSource.includes("border: `2px solid ${entry.border}`") && heroSource.includes("minHeight: 184") && !heroSource.includes("minHeight: 224") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep enough content density without returning to overly long sticky-note panels.");
