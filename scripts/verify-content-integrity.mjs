...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(280px, 0.68fr) minmax(600px, 1.32fr)"') && heroSource.includes("hero-featured-panel") && heroSource.includes("内容目录") && heroSource.includes("hero-entry-list"), "Hero must use one original-studio first-screen layout with a directly visible compact content directory.");
  ...
  expect(heroSource.includes(".hero-inner,") && heroSource.includes(".hero-featured-panel,") && heroSource.includes(".hero-entry-list") && !heroSource.includes(".hero-summary-panel,") && heroSource.includes("max-width: 100% !important"), "Homepage mobile layout must keep content inside the viewport instead of clipping at the right edge.");
  expect(heroSource.includes("grid-template-columns: 2.1rem minmax(0, 1fr) !important") && heroSource.includes("max-width: 100% !important") && heroSource.includes("gap: 0.5rem !important") && heroSource.includes("-webkit-line-clamp: 1 !important") && heroSource.includes("text-overflow: ellipsis !important"), "Homepage mobile rows must keep readable wrapped descriptions without right-edge overflow.");
  ...
  expect(heroSource.includes("hero-entry-desc") && heroSource.includes("-webkit-line-clamp: 1"), "Hero directory rows must show compact clamped module descriptions without becoming long pages.");
  expect(heroSource.includes('minHeight: "calc(100vh - 50px)"') && heroSource.includes('alignItems: "start"') && heroSource.includes('alignContent: "start"') && heroSource.includes('gridTemplateColumns: "minmax(280px, 0.68fr) minmax(600px, 1.32fr)"') && heroSource.includes("maxWidth: 1360") && !heroSource.includes('height: "calc(100vh - 116px)"'), "Homepage hero must keep the original landing feel while top-aligning first-screen content instead of centering empty slogan space.");
  expect(heroSource.includes('className="hero-entry-list"') && heroSource.includes('display: "grid"') && heroSource.includes('gap: "0.42rem"'), "Homepage module entries must render as a compact first-screen list, not repeated sections.");
  expect(heroSource.includes("borderLeft: `0.42rem solid ${entry.border}`") && heroSource.includes("minHeight: 70") && !heroSource.includes("minHeight: 158") && !heroSource.includes("minHeight: 224") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep enough content density without returning to overly long sticky-note panels.");
  expect(!heroSource.includes('gridRow: entry.featuredImage ? "span 2" : undefined') && !heroSource.includes("featuredImage") && heroSource.includes("hero-entry-side"), "Homepage plant evolution entry must stay a compact directory row instead of a two-row illustration card.");
  ...
  expect(heroSource.includes("hero-entry-path") && heroSource.includes("entry.path.map") && !heroSource.includes("entry.outputs.slice(0, 3)") && !heroSource.includes("hero-entry-task") && !heroSource.includes("task: work.task"), "Homepage rows must preserve useful learning-path density without restoring long task/output blocks.");
  ...
  expect(!heroSource.includes("hero-reading-library") && !heroSource.includes("阅读库 ·") && heroSource.includes('href: "/reading"'), "Homepage may include one compact reading-library directory row, but not a separate repeated reading block.");
...
function verifyWorkJsonLdLearningOutcomes() {
  ...
  expect(heroSource.includes("hero-entry-row") && heroSource.includes('display: "grid"') && heroSource.includes("hero-featured-panel") && !heroSource.includes("hero-work-card"), "Homepage hero module entries must use one featured directory, not repeated card sections.");
  ...
  expect(!heroSource.includes("hero-reading-library") && !heroSource.includes("hero-article-row") && heroSource.includes('href: "/reading"'), "Homepage hero may include one compact reading-library row but must not repeat article entrances on the first screen.");
  expect(!heroSource.includes("minHeight: 224") && !heroSource.includes("minHeight: 230") && !heroSource.includes("minHeight: 260") && !heroSource.includes("hero-work-summary"), "Homepage hero module entries must not use tall sticky-note cards.");
  expect(heroSource.includes(".hero-entry-list") && heroSource.includes("grid-template-columns: 2.1rem minmax(0, 1fr) !important") && heroSource.includes("padding: 0.48rem 0.56rem !important") && heroSource.includes(".hero-entry-side") && heroSource.includes("display: none !important"), "Homepage hero mobile featured directory must remain short while fitting the visible viewport.");
