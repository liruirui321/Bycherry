...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes("hero-article-row") && heroSource.includes("articleLinks.map") && heroSource.includes("href={article.href}"), "Homepage must still expose article routes inside the compact reading library.");
...
function verifyWorkJsonLdLearningOutcomes() {
  ...
  expect(heroSource.includes("hero-reading-library") && heroSource.includes("hero-article-row") && heroSource.includes('aria-label={`打开${article.title}：${article.desc}`}'), "Homepage hero must expose article routes only inside one compact reading library with accessible descriptions.");
  ...
  expect(heroSource.includes("display: grid !important") && heroSource.includes(".hero-directory-group") && !heroSource.includes("grid-template-columns: repeat(2, minmax(0, 1fr)) !important") && !heroSource.includes("min-height: 46px !important"), "Homepage hero mobile module directory must stay as a short grouped wrapping strip instead of stacked panels.");
