...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(320px, 0.72fr) minmax(0, 1.28fr)"') && heroSource.includes("hero-soft-blob") && heroSource.includes("hero-stem") && heroSource.includes("hero-float-deco") && heroSource.includes("hero-scope-strip"), "Hero must keep the original two-column studio composition with visible background atmosphere and useful left-side density.");
  ...
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(3, minmax(0, 1fr))"') && heroSource.includes('gridAutoRows: "minmax(188px, auto)"'), "Homepage module entries must render as a dense three-column first-screen directory, not repeated sections.");
  expect(heroSource.includes("border: `2px solid ${entry.border}`") && heroSource.includes("minHeight: 188") && !heroSource.includes("minHeight: 230") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep enough content density without returning to long sticky-note panels.");
  expect(!heroSource.includes('gridRow: entry.featuredImage ? "span 2" : undefined') && heroSource.includes('height: 88'), "Homepage plant evolution entry must show a compact illustration without becoming a two-row sticky-note card.");
