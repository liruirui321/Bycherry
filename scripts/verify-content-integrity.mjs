...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes("hero-entry-desc") && heroSource.includes("-webkit-line-clamp: 1") && heroSource.includes("-webkit-line-clamp: 2"), "Hero directory rows must show compact one-line module descriptions without becoming long cards.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(2, minmax(0, 1fr))"'), "Homepage module entries must render as one compact first-screen directory, not repeated sections.");
  expect(heroSource.includes('border: "none"') && heroSource.includes('borderLeft: `3px solid ${entry.border}`') && heroSource.includes("minHeight: 56") && !heroSource.includes("minHeight: 76") && !heroSource.includes("minHeight: 88"), "Homepage module entries must stay as short text links with compact descriptions instead of long sticky-note cards.");
...
function verifyWorkJsonLdLearningOutcomes() {
  ...
  expect(heroSource.includes("hero-entry-row") && heroSource.includes('display: "grid"') && heroSource.includes('border: "none"') && !heroSource.includes("hero-work-card"), "Homepage hero module entries must use one compact directory, not long repeated cards.");
  expect(heroSource.includes("hero-entry-desc") && !heroSource.includes("hero-entry-description-card"), "Homepage hero can expose module descriptions only inside compact directory rows.");
  ...
  expect(heroSource.includes(".hero-entry-grid") && heroSource.includes("grid-template-columns: repeat(2, minmax(0, 1fr)) !important") && heroSource.includes("padding: 0.38rem 0.42rem !important") && heroSource.includes(".hero-entry-action") && heroSource.includes("display: none !important"), "Homepage hero mobile module directory must remain short while fitting the visible viewport.");
...
function verifyLearnerFacingArticleCopy() {
  ...
  expect(articleLibrarySource.includes("article-library-meta") && articleLibrarySource.includes("article-library-date") && articleLibrarySource.includes("article-library-first-action") && articleLibrarySource.includes("先做：{article.firstAction}") && articleLibrarySource.includes("点标题进入正文"), "Reading library visible rows must show one compact first action so entries are immediately useful.");
