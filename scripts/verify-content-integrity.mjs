...
function verifyWorkCardActions() {
  ...
  expect(heroSource.includes('gridTemplateColumns: "minmax(330px, 0.78fr) minmax(420px, 1fr)"') && heroSource.includes("maxWidth: 2060") && heroSource.includes('alignItems: "start"'), "Homepage header must stay near the top and use wide-screen space instead of leaving large empty margins.");
  expect(heroSource.includes('className="hero-entry-grid"') && heroSource.includes('gridTemplateColumns: "repeat(3, minmax(0, 1fr))"') && heroSource.includes('gridAutoRows: "minmax(224px, 1fr)"'), "Homepage module entries must render as a compact three-column first-screen matrix, not repeated sections.");
  expect(heroSource.includes("border: `2px solid ${entry.border}`") && heroSource.includes("minHeight: 224") && !heroSource.includes("minHeight: 260"), "Homepage module entries must keep enough content density without returning to overly long sticky-note panels.");
