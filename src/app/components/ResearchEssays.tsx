...
export const essays = [
  ...
  },
  {
    ...
    body: "平台链接：https://scifuion.top。打开后先确定测评用途，再输入课程对象、知识点、题型、难度和题量；生成后逐题审核题干、答案、干扰项和解析。",
    readMin: 8,
    tags: ["SciFuion", "智能测评", "使用指南"],
    platformUrl: "https://scifuion.top",
    paragraphs: [
      "平台链接：https://scifuion.top。这个页面的用法很直接：先把它当作测评题目生产台，再把自己当作最终审核人。平台负责快速生成题目，人负责判断这些题目能不能真的用于课堂。",
      "第一步先定用途。课前诊断只需要 3-5 道题，用来判断学生有没有进入新课的基础；课中检查适合 1-2 道概念辨析题，用来发现刚讲完的误解；课后复习可以设置 6-10 道题，让学生把概念、过程和证据串起来。",
      "第二步写清楚测评对象和目标。不要只写“生物必修二”或“基因表达”，而要写成学生能完成的动作，例如“学生能区分转录产物和翻译产物”“学生能判断密码子与氨基酸的对应关系”。目标越具体，生成结果越容易审核。",
      "第三步填写生成范围：课程对象、知识点范围、题型、难度、题量和预计完成时间。第一次建议少量生成，先用 5-8 道题跑通流程。题量太大时，审核成本会变高，题目质量也更容易被忽略。",
      "第四步审核题干。每道题只问一个核心问题，条件要足够支撑答案。如果题目涉及图表、实验、案例或数据，要确认学生不需要靠猜测教师意图作答。题干不清楚时，宁可重写，不要直接投放。",
      "第五步审核答案和干扰项。正确答案必须唯一；干扰项要来自真实误解，而不是随便写一个明显错误的选项。例如基因表达主题里，可以把 RNA 聚合酶和 DNA 聚合酶混淆、把 mRNA 和蛋白质产物混淆作为干扰项。",
      "第六步审核解析。合格解析要说明为什么这个答案成立，也要说明其他选项为什么不成立，并指出对应知识点。解析如果只是重复答案，就还不能帮助学生纠错。",
      "第七步投放和复盘。投放后不要只看平均分，要看高频错题和高频错因。多人错同一题时，先判断题目表达是否有歧义，再判断是不是课堂讲解或活动设计没有把概念讲透。",
      "一个可直接套用的例子：主题写“基因表达”，对象写“高中生物，已经学过 DNA 和 RNA 基础”，目标写“学生能说明 DNA、mRNA、蛋白质之间的信息传递关系”，题型选选择题和简答题，题量 6 题，难度中等，完成时间 8 分钟。",
    ],
    highlights: ["平台链接：https://scifuion.top", "输入对象、目标、知识点、题型、难度和题量", "生成后先人工审核，再投放和复盘"],
    actionSteps: ["打开 https://scifuion.top", "选择本次用途：课前诊断、课中检查或课后复习", "填写课程对象：年级、学科、章节和学生基础", "写下一个可观察目标：学生需要证明自己能做什么", "填写知识点、题型、难度、题量和预计完成时间", "生成题目后逐题审核题干、答案、干扰项和解析", "按平台提供的发布或导出方式投放给学生", "记录高频错因，并据此修改题目或补充讲解"],
    checklist: ["平台链接已经给到使用者：https://scifuion.top", "测评目标写成学生可完成的具体动作", "每道题只考一个核心问题，条件足够支撑答案", "正确答案唯一，干扰项对应真实误解", "解析说明了为什么对、为什么错、对应哪个知识点", "题量和完成时间符合课堂节奏", "测后复盘了高频错因，而不是只看平均分"],
    starterTemplate: ["平台入口：https://scifuion.top", "使用场景：课前诊断 / 课中检查 / 课后复习", "课程对象：年级、学科、章节、学生基础", "测评目标：学生需要证明自己能……", "知识点范围：本次只覆盖……", "题目设置：题型、难度、题量、预计完成时间", "生成要求：题干清楚，答案唯一，干扰项来自真实误解，解析说明原因", "人工审核：删掉跑题、歧义、超纲或解析空泛的题", "投放复盘：最高频错因是什么，下一步改题还是补讲"],
    pitfalls: ["不要把平台当成自动出卷后直接发布的黑箱", "不要只写章节名，要写清楚学生需要完成的动作", "不要只检查答案而忽略题干歧义和解析质量", "不要把所有错因都归为学生粗心", "不要一次生成太多题，先用小题量把流程跑顺"],
  },
];
