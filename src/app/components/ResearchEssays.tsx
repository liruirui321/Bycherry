...
export const essays = [
  ...
  },
  {
    ...
    labelColor: "#3C2D6E",
    labelBg: "#EDE9F5",
    title: "SciFuion 智能测评平台怎么用",
    body: "平台入口：https://scifuion.top。适合把一次课程内容快速整理成诊断题、随堂题或复习题；使用时先定目标，再生成题目，最后人工审核解析和错因。",
    readMin: 9,
    tags: ["SciFuion", "智能测评", "使用指南"],
    platformUrl: "https://scifuion.top",
    paragraphs: [
      "平台入口：https://scifuion.top。它更适合当作“测评生产台”来用：把课程目标、知识点和学生对象输入进去，快速得到一组可审核、可投放、可复盘的题目。不要把它当成自动出卷后直接发布的黑箱。",
      "开始前先选清楚使用场景。课前诊断要短，重点看学生是否具备进入新课的基础；课中检查要尖锐，重点抓刚讲完的概念误解；课后复习可以稍综合，要求学生把概念、过程和证据串起来。",
      "填写测评目标时，尽量写成可观察动作。例如“学生能判断 DNA、mRNA、蛋白质三者在基因表达中的关系”，比“考基因表达”更好。平台需要明确目标，教师也需要用这个目标来审核题目有没有跑偏。",
      "接着设定生成范围：知识点、适用对象、题型、难度、题量。知识点决定边界，适用对象决定语言深浅，题型决定课堂用法，难度决定是否需要推理，题量决定学生完成时间。第一次使用建议少量生成，先做 5-8 题。",
      "生成题目后先看题干。题干要只问一个核心问题，不能同时混入两个判断；条件要完整，不能让学生靠猜教师意图作答；如果题目涉及图表、实验或情境，要确认信息足够支持答案。",
      "再看选项和答案。正确答案必须唯一、稳定；干扰项最好来自真实误解，例如把 RNA 聚合酶和 DNA 聚合酶混淆、把转录和翻译产物混淆。干扰项不能只是明显错误的废话，否则测不出学生的问题。",
      "解析是最需要人工把关的部分。合格解析应说明正确答案为什么成立，错误选项为什么不成立，并点出对应知识点。若解析只是重复答案，或者用了超出课程范围的术语，就需要改写后再投放。",
      "投放给学生时，可以按三种方式使用：课前发 3-5 道诊断题，课中插入 1-2 道概念辨析题，课后布置一组复习题。每一种用法都要控制时长，让测评服务课堂节奏，而不是把课堂拖成刷题。",
      "测后要看错因，而不只是看分数。多人错同一题时，先判断题目表达是否清楚，再判断教学中是否没有讲透，最后才判断学生是否没有复习。真正有价值的是用错因反向修改题目、讲解和活动设计。",
    ],
    highlights: ["入口：https://scifuion.top", "按课前、课中、课后选择测评目标", "审核题干、答案、干扰项和解析后再发布"],
    actionSteps: ["打开 https://scifuion.top", "确定本次用途：课前诊断、课中检查或课后复习", "写下一个可观察目标：学生需要证明自己能做什么", "填写知识点、对象、题型、难度和题量，第一次建议生成 5-8 题", "逐题审核题干、答案、干扰项和解析", "投放后记录高频错因，并用错因修改题目或补充讲解"],
    checklist: ["平台链接已经给到使用者：https://scifuion.top", "测评目标不是大标题，而是学生可完成的具体动作", "每道题只考一个核心问题，条件足够支撑答案", "正确答案唯一，干扰项对应真实误解", "解析解释了为什么对、为什么错、对应哪个知识点", "测后复盘了高频错因，而不是只看平均分"],
    starterTemplate: ["平台入口：https://scifuion.top", "使用场景：课前诊断 / 课中检查 / 课后复习", "课程对象：年级、课程、学生基础", "测评目标：学生需要证明自己能……", "知识点范围：本次只覆盖……", "题目设置：题型、难度、题量、预计完成时间", "人工审核：题干是否清楚，答案是否唯一，干扰项是否有效，解析是否讲清楚", "投放复盘：最高频错因是什么，下一步改题还是补讲"],
    pitfalls: ["不要直接生成整套题就发布", "不要只检查答案而忽略题干歧义和解析质量", "不要把所有错因都归为学生粗心", "不要一次生成太多题，先用小题量把流程跑顺"],
  },
];
