...
export const notes = [
  {
    ...
    readTime: "7",
    paragraphs: [
      "AI 适合做低风险的材料整理：拆概念、列先修知识、生成对比表、改写练习。但这些输出只能节省时间，不能自动证明你已经理解。",
      "我会先让 AI 分别列出学习目标、先修知识、常见误解和练习顺序，再把结果当草稿来审。它提供备选方案，不提供最终判断。",
      "真正困难的是判断材料是否符合学科逻辑、是否会造成误解、是否解释了自己的卡点。这个判断必须回到证据、例子和自己的输出。",
      "以“基因表达”为例，AI 很容易列出转录、翻译、密码子和蛋白质合成。你要判断自己卡在信息如何传递，还是结构如何对应功能。",
      "更稳的做法是给 AI 小任务：列混淆对象、写可观察证据、生成自测问题、解释错因。任务拆开后，哪部分可靠、哪部分要重写会更清楚。",
      "最后把 AI 输出分成三类：可以直接复习的表达，可以改写后使用的练习，必须回到教材或资料核查的判断。理解要由自己的解释、练习和复盘证明。",
    ],
  ...
  },
  {
    ...
    readTime: "7",
    paragraphs: [
      "项目制学习最容易变成“活动很多，但学习不清楚”。我会先问：最终作品是什么，它能证明自己学到了什么？不能承载证据的作品，很容易只剩热闹。",
      "作品不一定复杂，但必须能看出理解。调查报告、模型图、实验方案、科学短片或展板，都要回答：用了哪些资料，怎样组织证据，结论是否合理。",
      "任务流程要和评价量规一起设计。量规不是最后评分表，而是开始前的导航：什么算好、要留下什么证据、哪里需要修改。",
      "我会把量规写成可观察表现：问题是否清楚，证据是否可靠，解释是否连得上，局限是否说明，反馈后是否留下修订痕迹。",
      "项目过程每一步都要留下中间证据：问题草稿、资料卡、实验记录、同伴反馈和修订版本。过程材料能证明理解如何变化，而不是只展示最终成品。",
      "不知道从哪里开始时，先做一张项目证据表：任务节点、要留下的材料、它能证明什么。右列写不出来，就说明这个节点还没有真正服务学习目标。",
    ],
