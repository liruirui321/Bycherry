...
export const notes = [
  {
    ...
    paragraphs: [
      ...
      "我更常用的做法，是先给 AI 一个很窄的任务：不要让它设计完整一节课，而是让它分别生成“学生可能误解”“可观察证据”“课堂提问”“即时小测”。这些结果分开生成，更容易看出哪一部分可靠，哪一部分需要重写。",
      "落到一节“基因表达”课，可以这样使用：先让 AI 列出学生可能混淆的对象，如 DNA 聚合酶、RNA 聚合酶、核糖体、mRNA 和多肽链；再让它把每个误解改写成一道判断题；最后由教师检查这些题目是否真正对应课堂目标。",
      "如果 AI 给出的活动很多，我会用三个问题删减：这个活动能让学生做出可观察表现吗？它是否只服务一个核心目标？完成后有没有证据证明学生理解更清楚？三个问题答不上来，就先删掉。",
    ...
    actionSteps: ["先写清楚本节课学生最后要会做什么", "让 AI 分别生成先修知识、常见误解、课堂提问和即时小测", "把每个输出对应到一个可观察学生行为", "人工删掉超出学情和证据边界的内容", "用一节试讲或小测结果回头修改材料"],
    checklist: ["课程目标能用学生行为表述", "每个活动都能对应一个学习目标", "AI 输出经过学情和内容边界筛选", "即时小测能暴露真实误解", "材料修改有课堂反馈或小测证据"],
    starterTemplate: ["课程目标：学生能够……", "学生基础：已经会……，还容易混淆……", "AI 可协助：整理资料 / 生成变体 / 提供检查清单 / 改写小测", "人工必须判断：学情、误解风险、内容边界、任务时长", "课堂证据：学生完成……可以证明理解", "迭代证据：试讲观察 / 小测结果 / 学生反馈"],
    pitfalls: ["不要把 AI 生成的活动清单直接当课程设计", "不要用看起来丰富的材料掩盖学习目标不清", "不要让模型替你决定学生此刻能否理解", "不要一次让 AI 生成完整教案，先拆成小任务更容易审核"],
  },
  {
    id: 2,
    ...
    paragraphs: [
      ...
      "可以把耐旱性状做成一张课堂证据卡：左侧放两种植物在干旱处理前后的生长状态，右侧放一个简化表达图，显示某个胁迫响应基因在干旱条件下表达升高。学生的任务不是记住基因名，而是判断这张图支持什么解释。",
      "比较适合高中课堂的提问是：“这份证据能不能说明这个基因直接导致耐旱？”学生通常会先回答能，这时就可以引导他们区分相关性和因果性：表达升高说明它可能参与响应，但还需要敲除、过表达或更多样本来验证作用。",
      "如果材料来自论文，我会先删掉大部分方法细节，只保留学生必须理解的四类信息：研究对象是谁，比较条件是什么，数据有什么变化，作者的结论有没有限制。这样前沿材料才不会变成术语堆叠。",
    ...
    actionSteps: ["先选一个学生能观察到的植物性状或环境问题", "只截取一段能说明问题的真实数据或图表", "把材料改写成现象、证据、解释、限制四格", "让学生判断这份证据支持什么、不支持什么", "最后用一句话写出这个证据还缺什么验证"],
    checklist: ["入口问题不是纯术语", "证据材料能被学生读懂", "学生任务要求区分相关和因果", "结论和不确定性都被明确写出", "课堂不要求学生复述完整论文流程"],
    starterTemplate: ["进入问题：为什么……会出现差异？", "研究对象：比较的是……和……", "现象材料：学生能观察到……", "证据材料：图表/数据说明……", "可以支持的解释：这些证据提示……", "解释限制：这些证据还不能证明……", "下一步验证：还需要……"],
    pitfalls: ["不要从测序技术名词开始讲", "不要把相关性材料讲成因果证明", "不要一次塞进完整论文流程", "不要只讲科学家发现了什么，要让学生判断证据支持到哪一步"],
  },
  {
    id: 3,
    ...
    checklist: ["最终作品能证明学习目标", "量规在项目开始前已经给出", "过程证据和复盘修改能被追踪"],
    starterTemplate: ["驱动问题：我们要解决……", "最终作品：学生提交……", "评价量规：问题 / 证据 / 解释 / 表达 / 修改", "过程证据：草稿、资料卡、反馈、修订记录"],
    pitfalls: ["不要把活动数量当成项目质量", "不要等到最后才公布评价量规", "不要只看最终作品而忽略过程证据"],
  },
  {
    id: 4,
    ...
    checklist: ["角色和场景资产有固定设定", "每个生成步骤都有输入输出记录", "成片检查了连续性、声音和节奏"],
    starterTemplate: ["场景目标：这一镜头要表现……", "角色锁定：外观、服装、表情、禁用特征", "生成记录：输入、输出、问题、修正方式", "成片检查：节奏、连续性、声音、字幕"],
    pitfalls: ["不要先追求单张图惊艳再补流程", "不要频繁改角色设定", "不要把失败结果删掉而不记录原因"],
  },
];
